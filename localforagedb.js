/*
 localForageDb
 v 1.0 Okt 2015 Alexander Kuttig (http://www.xtheon.com)

 based on

 Kailash Nadh (http://nadh.in)
 localForageDB v 2.3.1
...
!(function (_global, undefined) {
    function localForageDB(db_name, callback) {
        var db_prefix = 'db_',
            db_id = null,
        ...
        storage.getItem(db_id, function(err, value) {
            if( !( value && (value = value) && value.tables && value.data ) ) {
                if(!validateName(db_name)) {
                    error("The name '" + db_name + "' contains invalid characters");
                } else {
                    db = {tables: {}, data: {}};
                    db_new = true;
                    commit(function(err, value){
                        db = value;
                        callback(err, db);
                    });
                }
        ...
        function drop(callback) {
            localforage.removeItem(db_id, function(err) {
                if(err == null){
                    db = null;
                    callback(null, true);
                }
        ...
        function commit(callback) {
            try {
                storage.setItem(db_id, db, function(err, value){
                    callback(err, value);
                });
            } catch(e) {
                callback("Error setting Item", null);
            }
        }
        ...
        return {
            ...
            create: function(db_name, callback){
                return create(db_name, callback);
            },
            ...
            createTableWithData: function(table_name, data, callback) {
    ...
    if(typeof define === 'function' && define.amd) {
        define(function() {
            return localForageDB;
        });
    } else {
        _global['localForageDB'] = localForageDB;
    }
